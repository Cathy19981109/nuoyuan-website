<template>
  <div>
    <div class="page-header">
      <div>
        <h2>服务筛选标签</h2>
        <p class="desc">维护前台服务筛选栏的标签项，支持拖拽排序</p>
      </div>
...
import {
  ...
  createServiceFilterGroup,
  updateServiceFilterGroup,
  deleteServiceFilterGroup,
  createServiceFilterTag,
  updateServiceFilterTag,
  deleteServiceFilterTag,
  reorderServiceFilterTags,
} from '@/api'
...
async function loadData() {
  const data = await getServiceFilterTags()
...
async function save() {
  ...
  try {
    if (editing.value) {
      await updateServiceFilterTag(editing.value.id, { tag_name: form.value.tag_name.trim() })
    } else {
      await createServiceFilterTag({ tag_group: form.value.tag_group, tag_name: form.value.tag_name.trim() })
    }
...
async function remove(group, item) {
  ...
  try {
    await deleteServiceFilterTag(item.id)
    await loadData()
  } catch (e) {
    alert(e.message)
  }
...
async function saveGroup() {
  ...
  try {
    if (editingGroup.value) {
      await updateServiceFilterGroup(editingGroup.value.id, { group_title: groupForm.value.group_title.trim() })
    } else {
      await createServiceFilterGroup({ group_title: groupForm.value.group_title.trim() })
    }
...
async function removeGroup(group) {
  ...
  try {
    await deleteServiceFilterGroup(group.id)
    await loadData()
  } catch (e) {
    alert(e.message)
  }
...
async function onDrop(group, targetId) {
  ...
  try {
    await reorderServiceFilterTags(group, { orderIds: rows.map((i) => i.id) })
  } catch (e) {
    alert(e.message)
    await loadData()
  } finally {
