...
function getTemplatesByPage(pageKey) {
  const common = [
    ...
    { code: 'multi_image_carousel', name: '多图轮播模块', imageRule: '16:9，1920x720，png/jpg/webp，<=50MB（逐张上传，可拖拽排序）' },
...
async function listModules(pageKey, includeHidden = false) {
  ...
  const [rows] = await pool.query(
    `SELECT * FROM nuoyuan_page_module WHERE page_key = ? ${condition} ORDER BY sort ASC, module_no ASC`,
    [pageKey]
  );
...
async function createModule(data, adminId) {
  ...
  const structured = toModuleForWrite(data);
  const [result] = await pool.query(
    `INSERT INTO nuoyuan_page_module
    (page_key, module_no, module_name, module_template, main_title, sub_title, body_text, layout_mode, image_list_json, video_url, link_url, jump_type, jump_product_code, table_text, qa_question, qa_answer, card_items_json, extra_json, front_position, module_data, sort, status, created_admin_id, updated_admin_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.page_key,
      moduleNo,
      data.module_name,
      data.module_template,
...
async function updateModule(id, data, adminId) {
  const fields = [];
  const values = [];
  const allowed = ['module_name', 'module_template', 'front_position', 'status'];
  allowed.forEach((key) => {
  ...
  if (structuredInputKeys.some((key) => data[key] !== undefined)) {
    const current = await getModuleById(id);
    const structured = toModuleForWrite({
      ...current,
      ...data,
    });
