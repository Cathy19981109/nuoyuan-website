/** 省市区简化数据：省份 → 城市；直辖市/港澳仅展示对应城市 */
export const MUNICIPALITIES = new Set([
  '北京市',
  '上海市',
  '天津市',
  '重庆市',
...
export const CHINA_REGIONS = [
  ...
  {
    name: '台湾省',
    cities: ['台北市', '高雄市', '台中市', '台南市', '新北市', '桃园市'],
  },
  {
    name: '内蒙古自治区',
...
export function isMunicipality(provinceName) {
  return MUNICIPALITIES.has(String(provinceName || ''))
}

export function citiesOfProvince(provinceName) {
  const row = CHINA_REGIONS.find((item) => item.name === provinceName)
  return row?.cities || []
}
